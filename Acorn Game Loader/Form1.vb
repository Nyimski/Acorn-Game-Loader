...
Public Class Form1
    ...
    Private controlFilePath As String = Path.Combine(Path.GetTempPath(), "uef_control.txt")
    ...
    Private Sub TerminateAllChildProcesses()
        ...
        If uefPlayProcess IsNot Nothing Then
            Try
                Try
                    If Not uefPlayProcess.HasExited Then
                        uefPlayProcess.Kill()
                    End If
                Catch ex As InvalidOperationException
                Catch ex As NotSupportedException
                Catch ex As Exception
                    Debug.WriteLine($"Error stopping uefPlayProcess: {ex.Message}")
                End Try
            Finally
                uefPlayProcess?.Dispose()
                uefPlayProcess = Nothing
                If _childProcesses.Contains(uefPlayProcess) Then
                    _childProcesses.Remove(uefPlayProcess)
                End If
            End Try
        End If
    ...
    Private Sub BtnPlay_Click(sender As Object, e As EventArgs) Handles BtnPlay.Click
        If uefPlayProcess Is Nothing OrElse uefPlayProcess.HasExited Then
            ...
            Try
                ...
                AddHandler uefPlayProcess.ErrorDataReceived, Sub(s, args)
                                                                 If Not String.IsNullOrEmpty(args.Data) Then
                                                                     Debug.WriteLine("Python Error: " & args.Data)
                                                                 End If
                                                             End Sub

                AddHandler uefPlayProcess.OutputDataReceived, Sub(s, args)
                                                                  If Not String.IsNullOrEmpty(args.Data) Then
                                                                      Debug.WriteLine("Python Output: " & args.Data)
                                                                  End If
                                                              End Sub

                uefPlayProcess.BeginErrorReadLine()
                uefPlayProcess.BeginOutputReadLine()
